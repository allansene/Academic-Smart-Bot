    ...
    public class ConversationDialog : IDialog
    {
        ...
        public async Task MessageReceivedAsync(IDialogContext context, IAwaitable<Message> argument)
        {
            ...
            else
            {
                ...
                if (!welcome)
                {
                    await WelcomeBack(context, name);
                }
                else if ((message.Text.ToLower().Equals("hi")) || (message.Text.ToLower().Equals("hello")))
                {
                    await HelloAgain(context);
                }
                else
                {
                    await InterpretQuery(context, message);
                }
            }
        }

        private async Task InterpretQuery(IDialogContext context, Message message)
        {
            //Interpret query
            AcademicResult resp = await AcademicKnowledgeApi.Interpret(message.Text.ToLower());
            if (resp.interpretations.Count == 0)
            {
                resp = await AcademicKnowledgeApi.Interpret(message.Text.ToLower());
            }
            ...
            else
            {
                ...
                string responseMessage = GetProperTextForEachInterpretation(resp);
        ...
        private async Task HelloAgain(IDialogContext context)
        {
            await context.PostAsync("Hello again!");
            context.Wait(MessageReceivedAsync);
        }

        private async Task WelcomeBack(IDialogContext context, string name)
        {
        ...
        private string GetProperTextForEachInterpretation(AcademicResult resp)
        {
            //TODO: check resp
        ...
        {
            var atts = attributes.GroupBy(g => g.Attribute).Select(grp => grp.ToList()).ToList();
            //  .OrderBy(x => x.Attribute).ToList<Item>();
        ...
        public async Task AfterChosenAsync(IDialogContext context, IAwaitable<long> argument)
        {
            ...
            if (result != null)
            {
                ...
                {
                    EvaluateResult resp = await AcademicKnowledgeApi.Evaluate(
                        result.interpretations[Convert.ToInt16(choice) - 1].rules[0].output.value);
                    int counter = 1;
